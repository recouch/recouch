...
describe('database functions', () => {
  ...
  describe('deleteDatabase', () => {
    it('deletes the database by reference', () => {
      ...
      expect(() => deleteDatabase(db)).not.toThrow()
    ...
    it('deletes the database by name', () => {
      ...
      expect(() => deleteDatabase(dbName, testDirectory)).not.toThrow()
      expect(fs.existsSync(dbPath)).toBe(false)
    ...
    it('does not throw when database does not exist', () => {
      expect(() => deleteDatabase(`non-existent-db-${nanoid()}`, testDirectory)).not.toThrow()
    })
  ...
  describe('beginTransaction/endTransaction', () => {
    it('commits all changes at once when committing a transaction', () => {
      ...
      expect(getDocument(db, 'doc1')).toBeUndefined()
      expect(getDocument(db, 'doc2')).toBeUndefined()
      ...
      expect(getDocument(db2, 'doc1')).toBeUndefined()
      expect(getDocument(db2, 'doc2')).toBeUndefined()
